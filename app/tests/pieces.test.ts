...
describe('déclarations des pièces', () => {
  it('le salon déclare cinq commandes, dont Porte, Rideau et Ambilight', () => {
    const libelles = PIECES.salon.commandes.map((c) => c.libelle);
  ...
  it('le salon ne déclare plus aucune ambiance : la rangée a cédé sa place aux commandes', () => {
    expect(PIECES.salon.ambiances).toEqual([]);
  });
  ...
  it('Porte ET Rideau sont épinglés, dans cet ordre', () => {
    expect(PIECES.salon.commandes.filter((c) => c.epingle).map((c) => c.libelle))
      .toEqual(['Porte', 'Rideau']);
  });

  it('Porte et Rideau passent par les scripts existants, jamais par lock/cover en direct', () => {
    const porte = PIECES.salon.commandes.find((c) => c.libelle === 'Porte')!;
    const rideau = PIECES.salon.commandes.find((c) => c.libelle === 'Rideau')!;
  ...
  it('la source Télévision déclare les quatre entités aux bons rôles', () => {
    const tv = PIECES.salon.sources.find((s) => s.nom === 'Télévision')!;
  ...
  it('la source Musique pointe partout vers la même entité', () => {
    const m = PIECES.salon.sources.find((s) => s.nom === 'Musique')!;
  ...
  it('les trois pièces déclarent leurs ouvrants et leur aspirateur', () => {
    expect(PIECES.salon.ouvrants).toContain('binary_sensor.porte_balcon_s_ouverture');
    expect(PIECES.salon.aspirateur).toBe('vacuum.aspirateur_cuisine');
    expect(PIECES.cuisine.ouvrants).toContain('binary_sensor.fenetre_c_ouverture');
  });

  it('la voiture et l\'entretien sont marqués perso, jamais la porte', () => {
    const par = (e: string) => PIECES.salon.synthese.find((s) => s.entite === e)!;
  ...
  it('la cuisine remplace la tuile aspirateur commune par le script du segment cuisine (19), jamais vacuum.start', () => {
    const a = PIECES.cuisine.aspirateurMaison!;
    ...
    expect(PIECES.salon.aspirateurMaison).toBeUndefined();
    expect(PIECES.bureau.aspirateurMaison).toBeUndefined();
  });
  ...
  it('chaque pièce déclare exactement le bloc par défaut attendu', () => {
    expect(PIECES.salon.blocDefaut).toBe('voiture');
    expect(PIECES.salon.voiture).toBeDefined();
    expect(PIECES.cuisine.blocDefaut).toBe('repas');
    expect(PIECES.bureau.blocDefaut).toBe('agenda');
  });
});
...
describe('tâche 19 — les quatre commandes ajoutées à la cuisine et au bureau', () => {
  ...
  it('la cuisine déclare quatre commandes, Hotte et Rideau en tête', () => {
    expect(PIECES.cuisine.commandes.map((c) => c.libelle))
  ...
  it('le rideau de cuisine passe par son script, jamais par cover.toggle', () => {
    const rideau = PIECES.cuisine.commandes.find((c) => c.libelle === 'Rideau')!;
  ...
  it('la hotte reste un simple light.toggle : aucun script ne porte de logique pour elle', () => {
    const hotte = PIECES.cuisine.commandes.find((c) => c.libelle === 'Hotte')!;
  ...
  it('la cuisine déclare todo.home_stock_shopping et plus aucune entité grocy', () => {
    expect(PIECES.cuisine.listesTachesExtra).toEqual(['todo.home_stock_shopping']);
    expect(JSON.stringify(PIECES.cuisine)).not.toMatch(/grocy/i);
  });
  ...
  it('la commande « Courses » ouvre la vue Tâches', () => {
    ...
    const b = PIECES.cuisine.commandes.find((c) => c.libelle === 'Courses')!;
  ...
  it('la cuisine a toujours QUATRE commandes', () => {
    ...
    expect(PIECES.cuisine.ambiances).toHaveLength(3);
  });
  ...
  it('la cuisine porte la ligne DLC, en CINQUIÈME position', () => {
    ...
    const s = PIECES.cuisine.synthese;
  ...
  it("le salon a la ligne DLC et RIEN d'autre du garde-manger", () => {
    ...
    const dlc = PIECES.salon.synthese.find((e) => e.entite === 'todo.home_stock_expirations');
    expect(dlc).toBeDefined();
    expect(dlc!.horsTaches).toBe(true);
    expect(PIECES.salon.listesTachesExtra).toEqual([]);
    expect(PIECES.salon.commandes.map((c) => c.entite))
  ...
  it("le bureau n'a aucune entité home_stock", () => {
    ...
    expect(JSON.stringify(PIECES.bureau)).not.toContain('home_stock');
  });
  ...
  it('« Aspirer ici » de l\'accueil est câblée EXACTEMENT comme celle de « Toute la maison »', () => {
    const accueil = PIECES.cuisine.ambiances.find((c) => c.libelle === 'Aspirer ici')!;
    const maison = PIECES.cuisine.aspirateurMaison!;
  ...
  it('purificateur et ventilateur basculent par fan.toggle, sans cible détournée', () => {
    const purificateur = PIECES.cuisine.ambiances.find((c) => c.libelle === 'Purificateur')!;
    const ventilateur = PIECES.bureau.commandes.find((c) => c.libelle === 'Ventilateur')!;
  ...
  it('le Velux est une tuile d\'état pure : ni service, ni cible, ni lien', () => {
    const velux = PIECES.bureau.commandes.find((c) => c.libelle === 'Velux')!;
  ...
  it('aucune pièce n\'affiche deux fois la même entité dans ses tuiles (commandes et ambiances)', () => {
    for (const [nom, piece] of Object.entries(PIECES) as [string, Piece][]) {
  ...
  it('aucune des quatre nouvelles entités n\'est déjà surveillée ailleurs sur SA tablette', () => {
    ...
    const ambiancesCuisine = PIECES.cuisine.ambiances.map((b) => b.entite);
    expect(ambiancesCuisine).toContain('fan.purificateur_air');
    const horsAmbiances = (p: Piece) => [
      ...p.synthese.map((e) => e.entite), ...p.ouvrants, p.temperature,
      ...(p.aspirateur ? [p.aspirateur] : []),
    ];
    expect(horsAmbiances(PIECES.cuisine)).not.toContain('fan.purificateur_air');
    expect(horsAmbiances(PIECES.cuisine)).not.toContain('light.hotte');
    expect(horsAmbiances(PIECES.cuisine)).not.toContain('cover.rideau_cuisine');
    expect(ailleurs(PIECES.bureau)).not.toContain('fan.chambre_ventilateur_tour');
    expect(ailleurs(PIECES.bureau)).not.toContain('binary_sensor.velux_ch_ouverture');
    ...
    expect(PIECES.bureau.ouvrants).toEqual([]);
  });
});
